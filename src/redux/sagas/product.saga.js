...
function* getProductListSaga(action) {
  try {
    const { limit, page, more, categoryFilter, keyword, priceFilter, sortFilter } = action.payload;
    let categoryParam = "";
    if(categoryFilter) {
    ...
    const result = yield axios.get(`http://localhost:4000/products?${categoryParam}`, {
      params: {
        _limit: limit,
        _page: page,
        ...(keyword && { q: keyword }),
    ...
    yield put({ type: FAIL(PRODUCT_ACTION.GET_PRODUCT_LIST), payload: { error: 'Error' } })
  }
}
...
function* getProductDetailSaga(action) {
  try {
    ...
    yield put({ type: FAIL(PRODUCT_ACTION.GET_PRODUCT_DETAIL), payload: { error: 'Error' } })
  }
}
