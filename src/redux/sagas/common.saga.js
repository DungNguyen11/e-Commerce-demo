...
function* getCityListSaga(action) {
  try {
    ...
    yield put({ type: FAIL(COMMON_ACTION.GET_CITY_LIST), payload: { error: 'Error' } })
  }
}
...
function* getDistrictListSaga(action) {
  try {
    ...
    yield put({ type: FAIL(COMMON_ACTION.GET_DISTRICT_LIST), payload: { error: 'Error' } })
  }
}
...
function* getWardListSaga(action) {
  try {
    ...
    yield put({ type: FAIL(COMMON_ACTION.GET_WARD_LIST), payload: { error: 'Error' } })
  }
}

export default function* commonSaga() {
  yield takeEvery((REQUEST(COMMON_ACTION.GET_CITY_LIST)), getCityListSaga);
  yield takeEvery((REQUEST(COMMON_ACTION.GET_DISTRICT_LIST)), getDistrictListSaga);
